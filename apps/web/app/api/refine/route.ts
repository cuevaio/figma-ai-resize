...
const SYSTEM_PROMPT = `You are a strict layout refinement generator for Figma frame resizing.
...
Process:
1. Build internal visual references from provided screenshots (do not output these references):
   - If 2 screenshots are provided, treat image[1] as original source-style reference and image[2] as current frame-to-refine.
   - If 1 screenshot is provided, treat it as current frame-to-refine.
...
2. Perform a deep internal diagnosis before proposing any edits:
   - Identify inconsistencies between original reference and current frame.
   - Identify what is wrong in the current frame stage (layout, readability, hierarchy, collisions, spacing rhythm, visual drift).
   - Rank issues by severity and user impact.
3. Use diagnosis findings to refine the already-adapted frame while preserving original look-and-feel.
4. Output only the final schema-compliant JSON.
...
const USER_PROMPT_TEMPLATE = `ROLE: Layout refinement generator.
TASK: Improve the already-adapted frame and produce full output JSON with target-frame-relative x/y/width/height and fontSize for each node.

FIRST STEP (INTERNAL, DO NOT OUTPUT):
- Build a compact visual style reference from the provided screenshot(s) before refining.
- If two screenshots are present, image[1] is the original source style reference and image[2] is the current frame-to-refine.
...
- Run a deep diagnosis before producing edits:
  - Inconsistencies vs origin (style, hierarchy, spacing, rhythm, emphasis)
  - Defects in current frame (off-frame placements, text overflow risks, non-allowed overlaps/collisions, weak title prominence, non-text proportion drift, parent-cluster incoherence)
  - Highest-severity issues to fix first
- Use this diagnosis to guide decisions in spacing, hierarchy, and emphasis during refinement.
...
- Highest-severity inconsistencies vs origin are addressed first.
...
export async function POST(request: Request) {
  ...
  const parsedRequest = parseRefineRequest(rawBody);
  if (parsedRequest === null) {
    return buildErrorResponse(
      requestId,
      startedAt,
      "BAD_REQUEST",
      "Invalid refine request payload.",
      400,
    );
  }

  const screenshotImage = tryGetScreenshotImageFromAnalysis(parsedRequest.input.analysis);
  if (screenshotImage === null) {
    return buildErrorResponse(
      requestId,
      startedAt,
      "BAD_REQUEST",
      "Refine request must include a screenshot image in input.analysis.screenshot (url or mimeType+base64).",
      400,
    );
  }

  const referenceScreenshotImage = tryGetScreenshotImage(parsedRequest.input.referenceScreenshot);
  ...
  try {
    ...
    if (referenceScreenshotImage !== null) {
      userContent.push({ type: "image", image: referenceScreenshotImage });
    }
    userContent.push({ type: "image", image: screenshotImage });
    ...
    console.info(
      `[api/refine] requestId=${requestId} runId=${parsedRequest.runId} status=200`,
    ...
    if (NoObjectGeneratedError.isInstance(error)) {
      return buildErrorResponse(
        ...
        startedAt,
        "UPSTREAM_PAYLOAD_ERROR",
        "Model response did not match refine schema.",
        502,
      );
    }
    ...
    const message = error instanceof Error ? error.message : "Unknown refine backend error.";
...
function buildErrorResponse(
  ...
  console.error(
    `[api/refine] requestId=${requestId} status=${status} code=${code}`,
    JSON.stringify(payload),
  );
...
function parseRefineRequest(value: unknown): RefineRequest | null {
  ...
  if (
    typeof value.input.referenceScreenshot !== "undefined" &&
    value.input.referenceScreenshot !== null &&
    tryGetScreenshotImage(value.input.referenceScreenshot) === null
  ) {
    return null;
  }
  ...
  return {
    ...
    input: {
      ...
      referenceScreenshot:
        typeof value.input.referenceScreenshot === "undefined"
          ? undefined
          : value.input.referenceScreenshot,
    },
    model,
  };
}
...
function tryGetScreenshotImageFromAnalysis(analysis: unknown): string | null {
  if (!isRecord(analysis)) {
    return null;
  }
  return tryGetScreenshotImage(analysis.screenshot);
}

function tryGetScreenshotImage(screenshot: unknown): string | null {
  ...
  return JSON.stringify({
    ...
    layoutHints: {
      ...
      visualReferenceFirstPass: {
        enabled: tryGetScreenshotImageFromAnalysis(analysis) !== null,
